...
namespace Clobscode
{
	...
	public:
		...
		virtual void operator=(const QuadEdge &e);
	...
	inline void QuadEdge::updateMidPoint(const unsigned int &idx){
		info[2] = idx;
	}

	inline unsigned int QuadEdge::operator[](unsigned const int &pos) const{
		return info.at(pos);
	}
