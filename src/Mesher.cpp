    ...
    void Mesher::splitQuadrants(const unsigned short &rl, Polyline &input,
        ...
        cout << "       * Refine Once Quad in "
             << std::chrono::duration_cast<chrono::milliseconds>(
                     end_refine_once_quad_time - start_refine_once_quad_time).count();
        cout << " ms" << endl;
...
//        tpv.setPoints(points);
        ...
        Quadrants.insert(Quadrants.end(), make_move_iterator(tmp_Quadrants.begin()),
                         make_move_iterator(tmp_Quadrants.end()));
        tmp_Quadrants.erase(tmp_Quadrants.begin(),
                            tmp_Quadrants.end()); // better to erase as let in a indeterminate state by move
    ...
    void Mesher::generateQuadtreeMesh(const unsigned short &rl, Polyline &input,
        ...
        for (int i = 1; i <= maxThread; ++i) {
            ...
            for (int i = 0; i < nbTries; ++i) {
                ...
                output[4].push_back(refineCustomMeshReductionTBBV3(i, tmp_Quadrants, points, QuadEdges, all_reg, rl, input));
                output[5].push_back(refineMeshCustomReductionTBBV4(i, tmp_Quadrants, points, QuadEdges, all_reg, rl, input));
                output[6].push_back(refineMeshReductionOpenMP(i, tmp_Quadrants, points, QuadEdges, all_reg, rl, input)); //V1
